...
Matrix4 CollisionDetection::GenerateInverseView(const Camera &c) {
	...
	pitch = -1.37000048;
	yaw = 319.290070;
	position = Vector3(-30, 40, 60);

	Matrix4 iview;
	Matrix4 tran = Matrix4::Translation(position);
	Matrix4 rotYaw = Matrix4::Rotation(yaw, Vector3(0, 1, 0));
	Matrix4 pitchYaw = Matrix4::Rotation(pitch, Vector3(1, 0, 0));

	iview = tran * rotYaw * pitchYaw;
...
bool NCL::CollisionDetection::CapsuleIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA, 
	...
	Vector3 capTopA(worldTransformA.GetPosition() + capExtPositionA);
	Vector3 capBottomA(worldTransformA.GetPosition() - capExtPositionA);

	Vector3 capExtPositionB = worldTransformB.GetOrientation() * Vector3(0, 1, 0) * (volumeB.GetHalfHeight() - volumeB.GetRadius());
	Vector3 capTopB(worldTransformB.GetPosition() + capExtPositionB);
	Vector3 capBottomB(worldTransformB.GetPosition() - capExtPositionB);
...
bool NCL::CollisionDetection::SphereOBBIntersection(const SphereVolume& volumeA, const Transform& worldTransformA,
	...
	sphereTransform.SetPosition(invTransform * worldTransformA.GetPosition());
	sphereTransform.SetScale(Vector3(1, 1, 1) * volumeA.GetRadius());

	AABBVolume aabb(volumeB.GetHalfDimensions());
	Transform aabbTransform;
	aabbTransform.SetPosition(invTransform * worldTransformB.GetPosition());
...
bool NCL::CollisionDetection::CapsuleOBBIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA, 
	...
	Vector3 point = worldTransformB.GetOrientation() * Maths::Clamp(
		invTransform * worldTransformA.GetPosition(),
		worldTransformB.GetPosition() - volumeB.GetHalfDimensions(),
		worldTransformB.GetPosition() + volumeB.GetHalfDimensions()
	);
